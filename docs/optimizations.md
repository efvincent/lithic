...
2. **Meta Tagging:** Expand `TMeta` to store its birth level: `TMeta Span Int Int` (Span, ID, Level). `freshMeta` must assign the current lexical level to new meta-variables.
