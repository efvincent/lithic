...
* **Status (May 2026):** Phase 10 C2 scaffold is merged (`feat/phase10-c-codegen`, v0.9.10.0). C2.1 (`feat/phase10-c2-emission`) landed typed signatures, literal/variable return emission, `let`-to-local lowering for `CPVar`, monomorphism guard diagnostics, and QQ-based interpolation helpers. C2.2 (`feat/phase10-c2-2-callcase-lowering`) is now merged to `main` via PR #28 with call/case/variant/select first-pass lowering contracts locked by tests in `test/Test/CGen.hs`. C4.1 CLI integration is landed (`lithic-cli --emit-c <input.lithic> [-o <output.c>]`), C4.2 compile-first validation gates are green in `Test.CGen` (`gcc -std=c11 -Wall -Wextra -Werror -c`) for representative emitted outputs, C4.3 fixture-level emit/compile integration is now landed in `Test.CLIEmitC` with dedicated declaration fixtures, C3.1 compile-time embedded prelude/resource migration is landed in `Compiler.CGen`, C3.2 first-pass runtime helper safety hardening is landed in `src/Compiler/CGenPrelude.c` (registered-boxed-handle validation before dereference), and C4.4 compile/link/run sanity checks now cover monomorphic identity, variant-helper, record-select, variant-case dispatch, unmatched fallback, wrong-kind, null-handle, and malformed non-zero handle runtime paths in `Test.CGen`. Next priority is deeper C3 runtime representation depth and helper-boundary contracts.
* **Scope (in):**
...
* **Tasks:**
  ...
  * [x] Move large static runtime C prelude/helper text from inline literals in CGen to a compile-time embedded template resource (executable-embedded; no runtime path lookup).
  ...
  * [ ] Validate emitted C compiles and runs correctly with `gcc -std=c11`. (C4.2 `gcc -c` gates are landed; C4.4 runtime sanity now covers positive plus negative wrong-kind/null/malformed-handle cases, and should continue expanding with helper contract depth.)
